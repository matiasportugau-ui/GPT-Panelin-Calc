...
curl -X POST http://localhost:3000/api/pdf \
  -H "Content-Type: application/json" \
  -d '{"cotizacion_id": "uuid-devuelto-por-api-cotizar", "cliente": {"nombre":"Test"}}' \
  --output prueba.pdf
```
...
| PDF vacío o error | `cotizacion_id` expirado o parámetros incompletos | Regenerar desde `/api/cotizar` y usar el nuevo `cotizacion_id`, o llamar `/api/pdf` con parámetros completos de cotización |
...
- Persistir cotizaciones o emitir tokens firmados para reducir la dependencia del cache en memoria en flujos críticos.
- Objetivo: evitar errores por reinicios o expiración implícita en entornos serverless.
- Entregable sugerido:
  - documentar que `cotizacion_id` y parámetros completos son las únicas opciones públicas de PDF
